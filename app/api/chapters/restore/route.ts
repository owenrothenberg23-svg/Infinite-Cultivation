...
export async function POST(req: Request) {
  try {
    ...
    const { data: restoredChapter, error } = await sb
      .from("chapters")
    ...
    if (story.is_public) {
      try {
        ...
        console.error("restore: catalog sync error", syncErr);
        return NextResponse.json(
          { error: "Chapter restored, but catalog chapter count could not be synced." },
          { status: 500 }
        );
      }
    }
